...
🔏 Edit your smart contract `HackathonMunon.sol` in `packages/hardhat/contracts`
...
✏️ Make small changes to `HackathonMunon.sol` and watch your app auto update!
...
> ⚗️ Spend some time tinkering with `HackathonMunon.sol`
...
📚 Go through each topic from [solidity by example](https://solidity-by-example.org) editing `HackathonMunon.sol` in **🏗 scaffold-eth**
...
🔲 Try making a `<Button/>` that calls `writeContracts.HackathonMunon.setPurpose("👋 Hello World")` to explore how your UI might work...
...
await tenderlyVerify(
  {contractName: "HackathonMunon",
   contractAddress: hackathonMunon.address
})
```
...
await run("verify:verify", {
  address: hackathonMunon.address,
